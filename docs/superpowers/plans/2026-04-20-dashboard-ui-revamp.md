...
**Goal:** Rewrite the Streamlit dashboard to show the full autonomous A2A workflow as a live transaction timeline, and fix the system prompt so a single user message triggers the full contract flow.

**Architecture:** The system prompt in `consumer/app.py` is patched to skip redundant catalog queries and auto-call `check_agreement_status` after requesting an agreement. The UI (`consumer/ui.py`) is fully rewritten: the right panel becomes a cumulative A2A transcript timeline grouped by phase (Catalog → Quote → On-chain TX → Gateway), built by parsing the existing `log` field from `/chat` responses client-side — zero new backend endpoints.

**Tech Stack:** Python, Streamlit, httpx, web3.py — no new dependencies.
...
| File | Change |
|---|---|
| `consumer/app.py` | Modify `SYSTEM_PROMPT` constant only (lines 219–227) |
| `consumer/ui.py` | Full rewrite |

---

## Task 1: Fix the system prompt in `consumer/app.py`

**Files:**
- Modify: `consumer/app.py:219-227`

**Root cause:** `SYSTEM_PROMPT` says "Always query the catalog first" — so the LLM calls `query_provider_catalog` on every turn even when the user names a tier directly. It also stops after `request_agreement_on_chain` and waits for the human instead of immediately calling `check_agreement_status`.

- [ ] **Step 1: Open and locate the constant**

  Open `consumer/app.py`. Find `SYSTEM_PROMPT` at line 219. The current text starts with `"You are a bandwidth procurement agent..."` and ends with `"NEVER guess, hallucinate, or use example numbers..."`.
...
- [ ] **Step 3: Verify the change looks right**
  ...
  Run:
  ```bash
  grep -n "SYSTEM_PROMPT" consumer/app.py
  ```
  Expected: two hits — the constant definition and its use in `run_consumer`.

- [ ] **Step 4: Smoke test via curl**

  Start the stack (`make up` or `docker compose up -d`), then:
  ```bash
  curl -s -X POST http://localhost:8001/chat \
    -H "Content-Type: application/json" \
    -d '{"message": "get small bandwidth", "model": "qwen3:1.7b"}' | python3 -m json.tool | grep -E '"response"|"from"'
  ```
  Expected: the `log` array should contain a `POST /quote package_id=small` entry AND a `requestAgreement() sent.` entry in the same response — no catalog query needed, no second prompt.

- [ ] **Step 5: Commit**

  ```bash
  git add consumer/app.py
  git commit -m "fix: system prompt skips catalog when tier is named, auto-checks agreement status"
  ```

---

## Task 2: Write phase parser utilities

**Files:**
- Modify: `consumer/ui.py` — add pure helper functions at the top (before the Streamlit app code)

These are pure functions with no Streamlit dependency, so they are easy to reason about in isolation before touching the UI.

- [ ] **Step 1: Add imports needed by the parser**

  At the top of `consumer/ui.py`, ensure these imports exist (add any missing):
  ```python
...
- [ ] **Step 2: Add constants**
  ...
  After the existing constants (`CONSUMER_BASE_URL`, `DEFAULT_MODEL`, `MODELS`), add:
...
- [ ] **Step 3: Add `_parse_log_to_phases`**
...
- [ ] **Step 5: Add `_current_step` and `_active_tier_from_timeline`**
...
- [ ] **Step 6: Quick sanity check (no Streamlit needed)**

  Run this one-liner to verify the parser against a synthetic log:

  ```bash
  python3 - <<'EOF'
  import sys; sys.path.insert(0, ".")
  from consumer.ui import _parse_log_to_phases, _merge_timeline, _current_step

  fake_log = [
      {"from": "consumer", "message": "GET /catalog"},
      {"from": "provider", "message": "small: 50 Mbps / 600s / 0.01 ETH (9 slots)\nmedium: 100 Mbps / 600s / 0.02 ETH (8 slots)"},
      {"from": "consumer", "message": "POST /quote package_id=small"},
      {"from": "provider", "message": "Quote received: agreementId=999, price=0.01 ETH"},
      {"from": "consumer", "message": "requestAgreement() sent. tx=0xabc, agreementId=999"},
      {"from": "consumer", "message": "Agreement ACTIVE. tokenId=0. Calling gateway..."},
      {"from": "provider", "message": "Gateway response: {'token_id': 0, 'bandwidth_mbps': 50}"},
  ]

  phases = _parse_log_to_phases(fake_log, turn=1)
  assert len(phases) == 4, f"expected 4 phases, got {len(phases)}: {[p['step'] for p in phases]}"
  assert phases[0]["step"] == "catalog"
  assert phases[1]["step"] == "quote" and phases[1]["summary"] == "0.01 ETH"
  assert phases[2]["step"] == "onchain" and len(phases[2]["messages"]) == 2
  assert phases[3]["step"] == "gateway"
  assert _current_step(phases) == "gateway"  # all done, returns last
  merged = _merge_timeline(phases, _parse_log_to_phases(fake_log, turn=1))
  assert len(merged) == 4, "merge should deduplicate same turn"
...
- [ ] **Step 7: Commit**
  ...
  git commit -m "feat: add phase parser utilities to ui.py"
  ```

---

## Task 3: Rewrite `consumer/ui.py` — rendering helpers + full layout

**Files:**
- Modify: `consumer/ui.py` — replace everything after the helper functions with new layout code

- [ ] **Step 1: Add `render_stepper`**

  Add this function after `_active_tier_from_timeline`:
...
- [ ] **Step 4: Replace the Streamlit app entry point**

  Delete everything from `st.set_page_config(...)` to the end of the file and replace with:
...
- [ ] **Step 5: Verify the file has no syntax errors**
  ...
  ```bash
  python3 -m py_compile consumer/ui.py && echo "OK"
  ```
  Expected: `OK`

- [ ] **Step 6: Re-run the parser sanity check to confirm helpers still work**

  ```bash
  python3 - <<'EOF'
  import sys; sys.path.insert(0, ".")
  from consumer.ui import _parse_log_to_phases, _merge_timeline, _current_step, _active_tier_from_timeline
  ...
  phases = _parse_log_to_phases(fake_log, turn=1)
  assert len(phases) == 4
  assert _active_tier_from_timeline(phases) == "small"
  print("All assertions passed.")
  EOF
  ```
  Expected: `All assertions passed.`

- [ ] **Step 7: Start the app and do a full end-to-end demo run**

  ```bash
  make up   # or: docker compose up -d
  ```
  Open http://localhost:8501. Then:
  1. Type `i want bandwidth give me options` → stepper should show **Catalog** as done, right panel shows "1 — Catalog Discovery" with consumer→provider bubbles
  2. Type `get small` → in a single response, the right panel should show all 4 phases completing (Catalog skipped if already done, or re-fetched — Quote, On-chain TX, and Gateway all appear). Stepper pills should all be green.
  3. Verify the active_tier highlight on the catalog strip (small card gets green border).
  4. Type another message (`get medium`) — new phases for turn 2 should appear below the existing ones (cumulative).

- [ ] **Step 8: Commit**

  ```bash
  git add consumer/ui.py
  git commit -m "feat: rewrite dashboard with A2A transaction timeline and stepper"
  ```
